...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_future_questions(self):
        ...
        future_question = Question(
            question_text='¿Que sistema operativo es mejor?', pub_date=time)
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_no_question(self):
        ...
        response = self.client.get(reverse('pollos:index'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'No pollos are available')
    ...
    def test_two_furure_questions(self):
        ...
        """
        future_question_1 = create_question(
            question_text="My future question example #1", days=10)
        future_question_2 = create_question(
            question_text="My future question example #2", days=2)
        response = self.client.get(reverse("pollos:index"))
        self.assertQuerysetEqual(
            # basicamente esto verifica que no se haya guardado nada
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_future_question(self):
        ...
        future_question = create_question(
            question_text='Future question', days=30)
        url = reverse('pollos:detail', args=(future_question.id,))
    ...
    def test_past_question(self):
        ...
        past_question = create_question(
            question_text="Past question", days=-30)
        url = reverse("pollos:detail", args=(past_question.id,))
        response = self.client.get(url)
